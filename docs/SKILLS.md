...
| [`ai-codegen-smell-audit`](../skills/ai-codegen-smell-audit/SKILL.md) | ~10K (single-pass review against ten specific patterns; the heuristics live in the SKILL.md itself, no sub-agents)             | ~30 (per portfolio repo, before merging substantial AI-generated diffs) |     ~300K | Author-estimated, pending measurement                |
| [`security-audit`](../skills/security-audit/SKILL.md) | ~5K (Phase 0 orchestration; the per-phase sub-agents add ~50-100K Sonnet input each, gated for user approval) | ~10 (per portfolio repo, when shipping a security review)                |      ~50K | Multi-phase, gated; phases run incrementally, not in one shot                                                       |
